    ...
    for row in persons_rows:
        gallery_data.append({
            ...
            "data": {
                ...
                "profile_image": make_web_path(row['profile_image'])
            },
...
def set_profile_image():
    ...
    conn = data_manager.get_db_connection()
    cursor = conn.cursor()
    face_row = cursor.execute('SELECT id FROM faces WHERE face_path LIKE ?', (db_face_path_pattern,)).fetchone()
    if not face_row:
        conn.close()
        return jsonify({'status': 'error', 'message': 'A megadott arckép nem található az adatbázisban.'}), 404

    face_id = face_row['id']
    cursor.execute('UPDATE persons SET profile_face_id = ? WHERE name = ?', (face_id, person_name))
    conn.commit()
    conn.close()
