...
def add_manual_face():
    ...
    try:
        ...
        data_manager.add_face_to_db(image_id, person_id, face_location, face_filepath_abs, 0.0, None)
...
def delete_face():
    ...
    db_face_path_pattern = '%' + face_path_to_delete.strip('/')
    full_path_to_delete = os.path.join(os.getcwd(), face_path_to_delete.lstrip('/\\'))

    conn = data_manager.get_db_connection()
    cursor = conn.cursor()
    cursor.execute('DELETE FROM faces WHERE face_path LIKE ?', (db_face_path_pattern,))
    conn.commit()
