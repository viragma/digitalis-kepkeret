...
def add_manual_face():
    ...
    try:
        with Image.open(image_path_abs) as img:
            ...
            left, top = int(coords_percent['left'] * img_width), int(coords_percent['top'] * img_height)
            width, height = int(coords_percent['width'] * img_width), int(coords_percent['height'] * img_height)
        ...
        # Manuális hozzáadáskor a distance 0.0, és is_manual, is_confirmed = 1
        data_manager.add_face_to_db(image_id, person_id, face_location, face_filepath_abs, 0.0, None, is_manual=True, is_confirmed=True)
